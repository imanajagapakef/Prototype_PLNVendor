...
function friendly(m: string): string {
  if (/comment required/i.test(m))
    return "Komentar wajib diisi untuk revisi / perbaikan.";
  if (/forbidden|not your contract|not authenticated|no profile/i.test(m))
    return "Anda tidak berhak melakukan itu pada kontrak ini.";
  if (/unknown/i.test(m)) return "Kontrak atau aksi tidak dikenal.";
  return "Transisi ditolak.";
}
...
export async function uploadAction(contractId: string, formData: FormData) {
  ...
  const docType = String(formData.get("docType") || "Pendukung").slice(0, 40);
  if (!(entry instanceof File) || entry.size === 0) {
    redirect(`${base}?err=${encodeURIComponent("Belum ada file dipilih.")}`);
  }
  const file = entry;
  if (file.size > MAX_SIZE) fail("File terlalu besar (maks 5MB).");
  const lower = file.name.toLowerCase();
  if (!ALLOWED_EXT.some((e) => lower.endsWith(e)))
    fail("Hanya PDF, JPG, PNG, WebP yang boleh.");
  ...
  if (!user) fail("Belum masuk.");
  const { error: upErr } = await sb.storage
    .from("contract-docs")
    .upload(path, file, { contentType: file.type || "application/octet-stream" });
  if (upErr) fail("Unggahan ditolak.");
  ...
  if (dbErr) fail("Metadata ditolak.");
  revalidatePath(`/contracts/${contractId}`);
  redirect(`/contracts/${contractId}`);
}
